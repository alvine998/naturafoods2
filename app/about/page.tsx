    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative overflow-hidden">
        ...
        <div className="sticky top-[64px] z-20 border-y border-[#2D4A22]/10 bg-white/90 backdrop-blur-xl">
          <div className="mx-auto flex max-w-[1280px] items-center gap-2 overflow-x-auto px-4 py-3 sm:px-6 md:px-8 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <span className="hidden shrink-0 text-[10px] tracking-[0.18em] text-[#8B6F47] sm:block">{L.tocLabel}</span>
            {L.toc.map((label, i) => {
      ...
      <section id="chain" className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 sm:py-16 md:px-8 md:py-20">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Reveal delay={0.08} className="max-w-[38ch] text-[13px] leading-6 text-[#1a1a16]/60">{L.chainNote}</Reveal>
        </div>
        ...
        <Reveal className="mt-6 overflow-hidden rounded-[24px] border border-[#2D4A22]/10 bg-[#FFF7E8] md:flex">
          <img src={L.chainImage} alt="Cafe" className="h-56 w-full object-cover md:h-auto md:w-[46%]" />
          <div className="p-6 sm:p-8 flex flex-col justify-center">
            <div className="text-[11px] tracking-[0.14em] text-[#8B6F47]">{L.whyLabel}</div>
            <div className="mt-2 font-[var(--font-display)] text-[20px] leading-tight text-[#2D4A22]">{L.whyTitle}</div>
            <p className="mt-3 text-[13px] leading-6 text-[#1a1a16]/60">{L.whyDesc}</p>
            <Link href="/products" className="mt-5 inline-flex w-fit items-center gap-1.5 rounded-full bg-[#2D4A22] px-5 py-2.5 text-[11px] tracking-[0.14em] text-white">{L.whyCta} <ArrowRight className="h-3.5 w-3.5" /></Link>
          </div>
        </Reveal>
      </section>
      ...
      <section id="values" className="bg-[#F5EFE0]/60">
        <div className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 sm:py-16 md:px-8 md:py-20">
          <Reveal><p className="text-[11px] tracking-[0.24em] text-[#8B6F47]">{L.valuesEyebrow}</p><h2 className="mt-3 font-[var(--font-display)] text-[30px] sm:text-[36px] font-light text-[#2D4A22] md:text-[42px]">{L.valuesTitle}</h2></Reveal>
      ...
      <section id="origins" className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 sm:py-16 md:px-8 md:py-20">
        <Reveal><p className="text-[11px] tracking-[0.24em] text-[#8B6F47]">{L.originsEyebrow}</p><h2 className="mt-3 font-[var(--font-display)] text-[30px] sm:text-[36px] font-light text-[#2D4A22] md:text-[42px]">{L.originsTitle}</h2></Reveal>
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {L.origins.map((o, i) => {
            const src = o.img?.trim() ? o.img : null;
            return (
            <Reveal key={o.name} delay={i * 0.08} className="group overflow-hidden rounded-[24px] border border-[#2D4A22]/10 bg-white">
              <div className="aspect-[16/10] overflow-hidden bg-[#F5EFE0]">{src ? <img src={src} alt={o.name} className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.04]" /> : <div className="grid h-full w-full place-items-center bg-[#F5EFE0] text-[11px] tracking-[0.14em] text-[#8B6F47]">No image</div>}</div>
              <div className="p-6">
        ...
        <div className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 sm:py-16 md:px-8 md:py-20">
          <div className="grid gap-8 md:grid-cols-[1.05fr_0.95fr] md:items-center">
            <Reveal className="overflow-hidden rounded-[24px] bg-[#FFF7E8] border border-[#2D4A22]/10">
              <img src={L.whImage} alt="Warehouse" className="aspect-[4/3] w-full object-cover" />
              <div className="grid grid-cols-3 divide-x divide-[#2D4A22]/10 border-t border-[#2D4A22]/10 bg-white text-center">
                {L.whStats.map((st) => (
                  <div key={st.k} className="p-4"><div className="font-[var(--font-display)] text-[16px] font-medium text-[#2D4A22]">{st.k}</div><div className="text-[10px] tracking-[0.1em] text-[#8B6F47]">{st.v}</div></div>
                ))}
              </div>
            </Reveal>
            <div>
      ...
      <section className="bg-[#2D4A22] px-4 py-12 sm:px-6 sm:py-16 md:px-8 md:py-20">
        <div className="mx-auto max-w-[1280px] grid gap-8 md:grid-cols-[1.2fr_0.8fr] md:items-center">
          <div>
            <Reveal><p className="text-[11px] tracking-[0.2em] text-white/60">{L.ctaEyebrow}</p><h2 className="mt-3 font-[var(--font-display)] text-[28px] sm:text-[34px] font-light leading-none text-white md:text-[42px]">{L.ctaTitle}</h2><p className="mt-4 max-w-[52ch] text-[13px] leading-6 text-white/65">{L.ctaDesc}</p></Reveal>
          </div>
          <Reveal delay={0.1} className="rounded-[24px] bg-white p-6 sm:p-8">
            <div className="text-[11px] tracking-[0.14em] text-[#8B6F47]">{L.ctaCardLabel}</div>
            <div className="mt-2 text-[13px] leading-6 text-[#1a1a16]/60">{L.ctaCardDesc}</div>
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-4 text-[12px]"><a href={`mailto:${L.ctaEmail}`} className="underline decoration-[#2D4A22]/20 underline-offset-4 text-[#2D4A22]">{L.ctaEmail}</a><a href={L.ctaWhatsapp} className="underline decoration-[#2D4A22]/20 underline-offset-4 text-[#2D4A22]">WhatsApp</a></div>
          </Reveal>
        </div>
      </section>
