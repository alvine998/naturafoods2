...
export default function SocialMediaAdminPage() {
  ...
  const pathFor = (id: string) => `/admin/social-media/${encodeURIComponent(id)}`;
  const commitSort = (item: SocialMedia, sortIndex: number) => {
    s.setSocialMedia((prev: SocialMedia[]) => prev.map((x) => (x.id === item.id ? { ...x, sortIndex } : x)));
    void patchSortIndex(pathFor(item.id), sortIndex);
  };
  const reorder = (from: number, to: number) => {
    if (isFiltering) return;
    const next = renumberByMove(sortedAll, from, to);
    s.setSocialMedia(next);
    void persistSortIndexDiff(sortedAll, next, (x) => pathFor(x.id));
  };
  const moveItem = (item: SocialMedia, dir: -1 | 1) => {
            ...
            <TableWrap>
              <table className="w-full min-w-[760px] text-[12px]">
                ...
                <tbody className="divide-y divide-[#2D4A22]/10">
                  {paged.map((m: SocialMedia, i: number) => {
                    const links = (["instagram", "facebook", "tiktok"] as Platform[]).filter((p) => m[p]);
                    const absIdx = (page - 1) * PAGE_SIZE + i;
                    return (
                    <tr key={m.id} {...rowProps(absIdx)} className={rowClass(absIdx)}>
                       <td className="px-3 py-2">
                         <SortIndexCell
                           value={m.sortIndex}
                           disabled={isFiltering}
                           onCommit={(v) => commitSort(m, v)}
                           onMove={(dir) => moveItem(m, dir)}
