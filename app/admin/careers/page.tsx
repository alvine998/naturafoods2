...
import { Card, Field, Input, TextArea, TableWrap, Pagination, Toolbar, Empty, PAGE_SIZE } from "../_components";
...
export default function CareersPage() {
  ...
  const openEdit = (i: number) => { setF(s.jobs[i]); setEditIdx(i); setFormOpen(true); };
  ...
  const save = () => {
    ...
    if (editIdx !== null) s.setJobs((prev: Job[]) => prev.map((x, i) => i === editIdx ? item : x)); else s.setJobs((prev: Job[]) => [...prev, item]);
    closeForm();
  };
