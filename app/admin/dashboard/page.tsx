    ...
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <Card className="p-4 sm:p-5 h-fit lg:sticky lg:top-[72px]">
        ...
        <div className="mt-4 grid gap-3">
          ...
          <Field label="category"><select value={f.cat ?? "choco"} onChange={(e) => setF({ ...f, cat: e.target.value as any })} className="w-full rounded-xl border border-[#2D4A22]/15 bg-white px-3.5 py-2.5 text-[13px] focus:outline-none focus:border-[#2D4A22]/30"><option value="choco">choco</option><option value="matcha">matcha</option></select></Field>
          ...
          <Field label="img url"><Input value={f.img ?? ""} onChange={(e) => setF({ ...f, img: e.target.value })} placeholder="https://..." /></Field>
      ...
      <div className="grid gap-3">
        <div className="flex gap-2">
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search products…" className="w-full rounded-full border border-[#2D4A22]/12 bg-white px-4 py-2.5 text-[13px] placeholder:text-[#1a1a16]/30 focus:outline-none focus:border-[#2D4A22]/25" />
          <span className="hidden sm:inline-flex items-center rounded-full bg-white border border-[#2D4A22]/10 px-4 text-[11px] text-[#8B6F47]">{filtered.length}/{s.products.length}</span>
        </div>
    ...
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <Card className="p-4 sm:p-5 h-fit lg:sticky lg:top-[72px]">
        ...
        <div className="mt-4 grid gap-3">
          ...
          <Field label="excerpt"><TextArea value={f.excerpt ?? ""} onChange={(e) => setF({ ...f, excerpt: e.target.value })} rows={2} /></Field>
          <Field label="content (HTML)"><TextArea value={f.content ?? ""} onChange={(e) => setF({ ...f, content: e.target.value })} rows={5} /></Field>
      ...
      <div className="grid gap-3">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search articles…" className="w-full rounded-full border border-[#2D4A22]/12 bg-white px-4 py-2.5 text-[13px] placeholder:text-[#1a1a16]/30 focus:outline-none" />
        {filtered.length === 0 && <Empty msg={a.noData} />}
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <Card className="p-4 sm:p-5 h-fit lg:sticky lg:top-[72px]">
        ...
        <div className="mt-4 grid gap-3">
          <Field label="id"><Input value={f.id ?? ""} onChange={(e) => setF({ ...f, id: e.target.value })} /></Field>
          <Field label="title"><Input value={f.title ?? ""} onChange={(e) => setF({ ...f, title: e.target.value })} /></Field>
    ...
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <Card className="p-4 sm:p-5 h-fit lg:sticky lg:top-[72px]">
        ...
        <div className="mt-4 grid gap-3">
          <Field label="id"><Input value={f.id ?? ""} onChange={(e) => setF({ ...f, id: e.target.value })} /></Field>
          <Field label="title"><Input value={f.title ?? ""} onChange={(e) => setF({ ...f, title: e.target.value })} /></Field>
          <Field label="dept"><Input value={f.dept ?? ""} onChange={(e) => setF({ ...f, dept: e.target.value })} /></Field>
          <Field label="location"><Input value={f.loc ?? ""} onChange={(e) => setF({ ...f, loc: e.target.value })} /></Field>
          <Field label="type"><Input value={f.type ?? ""} onChange={(e) => setF({ ...f, type: e.target.value })} /></Field>
          <Field label="desc"><TextArea value={f.desc ?? ""} onChange={(e) => setF({ ...f, desc: e.target.value })} rows={3} /></Field>
          <div className="flex gap-2 pt-1"><button onClick={save} disabled={!f.title} className="rounded-full bg-[#2D4A22] px-5 py-2.5 text-[11px] tracking-[0.12em] text-white disabled:opacity-50">{a.save}</button>{editIdx !== null && <button onClick={() => { setF({}); setEditIdx(null); }} className="rounded-full border border-[#2D4A22]/15 px-5 py-2.5 text-[11px]">{a.cancel}</button>}</div>
        </div>
      </Card>
      <div className="grid gap-3">
