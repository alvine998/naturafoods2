...
export default function EducationPage() {
  ...
  const sortedAll = useMemo(() => sortBySortIndex(s.edu as Edu[]), [s.edu]);
  const filtered = useMemo(() => {
    const n = q.trim().toLowerCase();
    if (!n) return sortedAll;
    return sortedAll.filter((e) => `${e.title}`.toLowerCase().includes(n));
  }, [sortedAll, q]);
  useEffect(() => setPage(1), [q]);
  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const paged = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  const isFiltering = q.trim() !== "";
  const pathFor = (id: string) => `/admin/education/${encodeURIComponent(id)}`;
  const commitSort = (item: Edu, sortIndex: number) => {
    s.setEdu((prev: Edu[]) => prev.map((x) => (x.id === item.id ? { ...x, sortIndex } : x)));
  ...
  const moveItem = (item: Edu, dir: -1 | 1) => {
  ...
  const openEdit = (i: number) => { setF(s.edu[i]); setEditIdx(i); setFormOpen(true); setErr(null); setSlugTouched(true); };
  const closeForm = () => { setF({}); setEditIdx(null); setFormOpen(false); setErr(null); setSlugTouched(false); };
  const save = async () => {
    if (!f.title) return;
    const item: Edu = { id: String(f.id ?? Date.now().toString()), title: String(f.title), desc: String(f.desc ?? ""), img: String(f.img ?? ""), link: String(f.link ?? ""), sortIndex: toSortIndex(f.sortIndex) };
    setSaving(true); setErr(null);
    const isEdit = editIdx !== null;
    const originalId = isEdit ? s.edu[editIdx!]?.id : null;
    try {
      if (isEdit) {
        await apiFetch(`/admin/education/${encodeURIComponent(originalId!)}`, { method: "PUT", body: JSON.stringify(item) });
        s.setEdu((prev: Edu[]) => prev.map((x, i) => i === editIdx ? item : x));
      } else {
        await apiFetch("/admin/education", { method: "POST", body: JSON.stringify(item) });
        s.setEdu((prev: Edu[]) => [...prev, item]);
      }
      closeForm();
    } catch (e) {
      ...
      if (!status || status === 0 || code === "NETWORK_ERROR") {
        ...
        if (isEdit) s.setEdu((prev: Edu[]) => prev.map((x, i) => i === editIdx ? item : x));
        else s.setEdu((prev: Edu[]) => [...prev, item]);
        closeForm();
      } else {
  ...
  const remove = async (realIdx: number) => {
    if (!confirmAdminDelete("this education item")) return;
    const item = (s.edu as Edu[])[realIdx];
    const snap = [...s.edu];
    s.setEdu((prev: Edu[]) => prev.filter((_, idx) => idx !== realIdx));
    try {
      await apiFetch(`/admin/education/${encodeURIComponent(item.id)}`, { method: "DELETE" });
    } catch (e) {
      const status = (e as { status?: number })?.status;
      const code = (e as { code?: string })?.code;
      if (status && status !== 0 && code !== "NETWORK_ERROR") {
        s.setEdu(snap as Edu[]);
    ...
    <AdminShell counts={counts} labels={a.tabs as unknown as string[]}>
      ...
      {formOpen ? (
        ...
        <div className="mt-4 grid gap-3">
          <Toolbar q={q} setQ={setQ} total={s.edu.length} filtered={filtered.length} onAdd={openAdd} addLabel={`${a.add} ${a.tabs[6]}`} />
          {filtered.length === 0 ? <Empty msg={a.noData} /> : (
            <TableWrap>
              <table className="w-full min-w-[640px] text-[12px]">
                <thead className="bg-white text-[10px] tracking-[0.12em] text-[#8B6F47]"><tr><th className="px-3 py-3 text-left font-medium">Sort</th><th className="px-3 py-3 text-left font-medium">Title</th><th className="px-3 py-3 text-right font-medium">Actions</th></tr></thead>
                <tbody className="divide-y divide-[#2D4A22]/10">
                  {paged.map((e: Edu, i: number) => {
                    const realIdx = (s.edu as Edu[]).indexOf(e);
                    const absIdx = (page - 1) * PAGE_SIZE + i;
                    return (
                      <tr key={e.id + realIdx} {...rowProps(absIdx)} className={rowClass(absIdx)}>
                        <td className="px-3 py-2">
                          <SortIndexCell
                            value={e.sortIndex}
                            disabled={isFiltering}
                            onCommit={(v) => commitSort(e, v)}
                            onMove={(dir) => moveItem(e, dir)}
