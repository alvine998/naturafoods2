...
import { LayoutDashboard, Package, Newspaper, GraduationCap, Lightbulb, Briefcase, Mail, Users, Menu, X, ArrowLeft } from "lucide-react";
...
const ICONS = [LayoutDashboard, Package, Newspaper, GraduationCap, Lightbulb, Briefcase, Mail, Users] as const;
...
export default function AdminShell({ counts, labels, children }: Props) {
  ...
  const Sidebar = () => (
    <nav className="grid gap-1">
      {nav.map((n) => (
        ...
        >
          <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${n.active ? "bg-white/15 text-white" : "bg-[#FFFCF2] border border-[#2D4A22]/10"}`}><n.Icon className="h-3.5 w-3.5" /></span>
  ...
  return (
    <div className="flex h-screen h-[100dvh] flex-col overflow-hidden bg-[#FFFCF2]">
      <header className="shrink-0 z-30 border-b border-[#2D4A22]/10 bg-[#FFFCF2]/85 backdrop-blur-xl">
