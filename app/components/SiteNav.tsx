...
const PRODUCT_SUB = [
  { label: { en: "Chocolate", id: "Cokelat", zh: "巧克力" }, desc: { en: "Couverture callets", id: "Callets couverture", zh: "调温纽扣巧克力" }, href: "/products?cat=choco" },
  { label: { en: "Matcha", id: "Matcha", zh: "抹茶" }, desc: { en: "Uji & Yame stone-milled", id: "Giling batu Uji & Yame", zh: "宇治·八女石磨" }, href: "/products?cat=matcha" },
  { label: { en: "All Products", id: "Semua Produk", zh: "全部产品" }, desc: { en: "Full catalog", id: "Katalog lengkap", zh: "完整目录" }, href: "/products" },
];
...
export default function SiteNav() {
  ...
  const nav: { label: string; href: string; dropdown?: typeof PRODUCT_SUB }[] = [
    ...
    { label: locale === "id" ? "PRODUK" : locale === "zh" ? "产品" : "PRODUCTS", href: "/products", dropdown: PRODUCT_SUB },
      ...
      >
        <nav className="mx-auto flex h-[64px] max-w-[1440px] items-center justify-between gap-4 px-6 md:px-8">
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0">
            ...
            <img src="/logo.png" alt="NaturaFoods" className="h-7 w-auto object-contain md:h-14" />
          </Link>
                ...
                <div key={n.href} className="group relative">
                  <Link
                    ...
                    className={`relative flex items-center gap-1 rounded-full px-3.5 py-2 text-[11px] tracking-[0.16em] transition ${
                      isActive(n.href) ? "text-[#2D4A22]" : "text-[#2D4A22]/60 hover:text-[#2D4A22] hover:bg-[#2D4A22]/[0.06]"
                    }`}
                  >
                    {n.label}
            ...
            >
              <div className="flex h-[64px] items-center justify-between border-b border-[#2D4A22]/10 px-6">
                <Link href="/" onClick={() => setOpen(false)} className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] tracking-[0.18em] text-[#8B6F47]">MENU</span>
                </Link>
                <button
                  onClick={() => setOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6">
                <div className="grid gap-1">
                  {nav.map((n) =>
                    n.dropdown ? (
                      <div key={n.href} className="rounded-2xl border border-transparent has-[button[aria-expanded=true]]:border-[#2D4A22]/10 has-[button[aria-expanded=true]]:bg-white">
                        <div className="flex items-center">
                          <Link href={n.href} onClick={() => setOpen(false)} className={`flex-1 py-3.5 text-[13px] tracking-[0.14em] ${isActive(n.href) ? "text-[#2D4A22]" : "text-[#2D4A22]/80"}`}>
                            {n.label}
                          </Link>
                          <button
              ...
              <div className="border-t border-[#2D4A22]/10 bg-white px-6 py-4">
                <div className="text-[11px] leading-5 text-[#8B6F47]">PT NaturaFoods Distribusi<br />Jakarta · Surabaya · Bali</div>
              </div>
            </motion.div>
          </>
        )}
