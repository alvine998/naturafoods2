...
const FIELDS = [
  { key: "bannerEyebrow", label: "Eyebrow", multiline: false },
  { key: "bannerTitle1", label: "Title (line 1)", multiline: false },
  { key: "bannerTitleItalic", label: "Title (italic middle)", multiline: false },
  { key: "bannerTitle3", label: "Title (line after italic)", multiline: false },
  { key: "bannerDesc", label: "Description", multiline: true },
  { key: "heroBannerCta", label: "CTA label", multiline: false },
  { key: "heroVideoSrc", label: "Video URL (mp4/webm)", multiline: false },
  { key: "heroVideoPoster", label: "Poster image URL", multiline: false },
] as const;
...
function baseValue(loc: Locale, key: FieldKey): string {
  return (dict[loc] as unknown as Record<string, string>)[key] ?? "";
}
function readValues(loc: Locale): Record<FieldKey, string> {
  ...
  for (const k of ALL_FIELDS) {
    const overridden = (cur as Record<string, unknown>)[k];
...
export default function HeroSliderEditModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 p-3 backdrop-blur-sm sm:items-center sm:p-6" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-label="Edit HeroSlider" onClick={(e) => e.stopPropagation()} className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-[#2D4A22]/10 bg-white shadow-2xl">
        <div className="flex items-center justify-between gap-3 border-b border-[#2D4A22]/10 bg-white px-4 py-3 sm:px-5 sm:py-4">
          <div>
            ...
            <h2 className="mt-0.5 font-[var(--font-display)] text-[20px] font-light leading-none text-[#2D4A22] sm:text-[22px]">HeroSlider</h2>
        ...
        {/* Remount on locale change so the draft re-initializes from the new locale's overrides. */}
...
function Editor({ editLocale, onClose }: { editLocale: Locale; onClose: () => void }) {
  ...
  const save = () => {
    ...
    let next: Record<string, unknown> = { ...cur };
    for (const k of ALL_FIELDS) {
      const v = draft[k]?.trim() ?? "";
      if (!v) next = deleteKey(next, k);
      else next = deepSet(next, [k], v) as Record<string, unknown>;
    }
  ...
  const resetAll = () => {
    if (!confirm("Reset all HeroSlider overrides for this language? This cannot be undone.")) return;
    setLocaleOverrides(editLocale, {});
