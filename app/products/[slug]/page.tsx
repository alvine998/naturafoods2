...
export default function ProductDetailPage() {
  ...
  if (!product) return <PageShell><Breadcrumbs items={[{ label: "Products", href: "/products" }, { label: "Not found" }]} /><p className="py-12 text-center text-[#8B6F47]">Product not found.</p><Link href="/products" className="mx-auto mt-4 block w-fit rounded-full bg-[#2D4A22] px-6 py-2.5 text-[11px] tracking-[0.14em] text-white">Back to products</Link></PageShell>;
  return (
    <PageShell>
      ...
      <div className="mt-4 sm:mt-6 overflow-hidden rounded-[20px] sm:rounded-[24px] bg-white border border-[#2D4A22]/10">
        ...
        <div className="p-4 sm:p-6 md:p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href="/contact" className="inline-flex rounded-full bg-[#2D4A22] px-6 py-2.5 text-[11px] tracking-[0.14em] text-white hover:bg-[#1e3317]">Request price</Link>
            <Link href="/products" className="inline-flex rounded-full border border-[#2D4A22]/15 bg-[#FFFCF2] px-6 py-2.5 text-[11px] tracking-[0.14em] text-[#2D4A22]">View all products</Link>
          </div>
        </div>
      </div>
